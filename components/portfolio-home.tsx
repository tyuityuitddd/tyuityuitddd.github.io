...
import { copy, translated, localUrl, type Work, type Settings, type Category } from '@/lib/portfolio';
...
export default function PortfolioHome({works,settings}:{works:Work[];settings:Settings}) {
 ...
 <section className={`hero language-${lang}`} ref={hero} aria-label={t.disciplines}><div className="hero-stage">
  ...
  {heroWorks.map((work,i)=><a key={work.slug} href={`${localUrl(`work/${work.slug}/`)}?lang=${lang}`} className={`hero-art art-${['a','b','c','d'][i]}`} aria-label={`${t.view} — ${translated(work.title,lang)}`}><ArtworkImage image={work.images[0]} sizes="(max-width: 760px) 38vw, 26vw" alt={translated(work.title,lang)} fetchPriority={i===0?'high':undefined}/><span className="art-number">0{i+1} / TDDD</span></a>)}
