...
import { copy, translated, mediaUrl, localUrl, type Work, type Settings, type Category } from '@/lib/portfolio';
...
export default function PortfolioHome({works,settings}:{works:Work[];settings:Settings}) {
 ...
 <section className={`hero language-${lang}`} ref={hero} aria-label={t.disciplines}><div className="hero-stage">
  ...
  {heroWorks.map((work,i)=><a key={work.slug} href={`${localUrl(`work/${work.slug}/`)}?lang=${lang}`} className={`hero-art art-${['a','b','c','d'][i]}`} aria-label={`${t.view} — ${translated(work.title,lang)}`}><img src={mediaUrl(work.images[0].thumbnail||work.images[0].src)} alt={translated(work.title,lang)} fetchPriority={i===0?'high':undefined}/><span className="art-number">0{i+1} / TDDD</span></a>)}
 ...
 <div className="work-grid">{filtered.slice(0,limit).map((work,index)=>{
  const img=work.images[0];return <a key={work.slug} href={`${localUrl(`work/${work.slug}/`)}?lang=${lang}`} className={`work-card ${work.category==='illustration'?'portrait-card':'landscape-card'}`}>
   <div className="work-image"><img src={mediaUrl(img.thumbnail||img.src)} alt={translated(work.title,lang)} loading="lazy" decoding="async" width={img.width} height={img.height}/><span className="work-index">{String(index+1).padStart(2,'0')}</span>{work.images.length>1&&<span className="image-count">{work.images.length} {t.images}</span>}<span className="work-open" aria-hidden="true"><ArrowUpRight size={24}/></span></div>
   <div className="work-caption"><h3>{translated(work.title,lang)}</h3><span>{t[work.category]}</span></div>
  </a>;
 })}</div>
