...
export function JournalIndex({posts}:{posts:JournalPost[]}) {
 ...
 return <div id="top" className="detail-page journal-page"><Header lang={lang} onLanguage={changeLanguage} detail/><main id="main" className="section-wrap journal-main"><div className="journal-masthead"><p className="eyebrow">TDDD / WORK IN PROGRESS</p><h1>{t.title}<span className="accent">.</span></h1><p>{t.intro}</p>{lang!=='zh'&&<small>{t.original}</small>}</div>{posts.length>1&&<nav className="journal-timeline" aria-label={t.timeline}><h2>{t.timeline}</h2><p>{t.historyNote}</p><ol>{[...posts].reverse().map(post=><li key={post.slug}><a href={`${localUrl(`journal/${post.slug}/`)}?lang=${lang}`}><time dateTime={post.date}>{post.period||journalDate(post.date,lang)}</time><span>{post.title}</span></a></li>)}</ol></nav>}{posts.length?<JournalCards posts={posts} lang={lang}/>:<p className="empty-state">{t.empty}</p>}</main><Footer lang={lang}/></div>;
}
