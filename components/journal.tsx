...
export function JournalCards({posts,lang}:{posts:JournalPost[];lang:Language}) {
 ...
 return <div className="journal-list">{posts.map(post=><article key={post.slug} className="journal-card">
  <div className="journal-date"><time dateTime={post.date}>{post.period||journalDate(post.date,lang)}</time><span>{post.retrospective?t.retrospective:'GODOT / DEVLOG'}</span></div>
...
export function JournalPreview({posts,lang}:{posts:JournalPost[];lang:Language}) {
 const t=journalCopy[lang];return <section className="journal-preview section-wrap"><div className="section-heading"><h2>{t.latest}</h2><a className="pill-link" href={`${localUrl('journal/')}?lang=${lang}`}>{t.all}<ArrowUpRight size={18}/></a></div>{posts.length?<JournalCards posts={posts.slice(0,3)} lang={lang}/>:<p>{t.empty}</p>}</section>;
}
export function JournalIndex({posts}:{posts:JournalPost[]}) {
 ...
 return <div id="top" className="detail-page journal-page"><Header lang={lang} onLanguage={changeLanguage} detail/><main id="main" className="section-wrap journal-main"><div className="journal-masthead"><h1>{t.title}<span className="accent">.</span></h1><p>{t.intro}</p>{lang!=='zh'&&<small>{t.original}</small>}</div>{posts.length>1&&<nav className="journal-timeline" aria-label={t.timeline}><h2>{t.timeline}</h2><p>{t.historyNote}</p><ol>{[...posts].reverse().map(post=><li key={post.slug}><a href={`${localUrl(`journal/${post.slug}/`)}?lang=${lang}`}><time dateTime={post.date}>{post.period||journalDate(post.date,lang)}</time><span>{post.title}</span></a></li>)}</ol></nav>}{posts.length?<JournalCards posts={posts} lang={lang}/>:<p className="empty-state">{t.empty}</p>}</main><Footer lang={lang}/></div>;
}
...
export function JournalArticle({post,next}:{post:JournalPost;next:JournalPost|null}) {
 ...
 return <div id="top" className="detail-page journal-page"><Header lang={lang} onLanguage={changeLanguage} detail/><main id="main" className="section-wrap journal-article"><a className="back-link" href={`${localUrl('journal/')}?lang=${lang}`}><ArrowLeft size={17}/>{t.back}</a><article lang="zh-Hant"><header className="journal-article-head"><div className="journal-meta"><time dateTime={post.date}>{post.period||journalDate(post.date,lang)}</time><span>TDDD / GODOT</span></div><h1>{post.title}</h1>{post.retrospective&&<p className="journal-history-label">{t.retrospective} · {t.recorded} {post.recordedOn&&journalDate(post.recordedOn,lang)}</p>}<p className="journal-lede">{post.excerpt}</p><div className="journal-tags">{post.tags?.map(tag=><span key={tag}>{tag}</span>)}</div>{post.updated&&post.updated!==post.date&&<p className="journal-updated">{t.updated} <time dateTime={post.updated}>{journalDate(post.updated,lang)}</time></p>}</header>
