...
import { copy, translated, localUrl, mediaUrl, videoEmbed, safeVideoLink, type Work } from '@/lib/portfolio';

export default function WorkDetail({work,next}:{work:Work;next:Work|null}) {
 const [lang,changeLanguage]=useLanguage();
 const [selected,setSelected]=useState<number|null>(null);
 const [playing,setPlaying]=useState(false);
 const t=copy[lang];const title=translated(work.title,lang);const embed=videoEmbed(work.video);const externalVideo=safeVideoLink(work.video);
 ...
 <main id="main"><section className="detail-intro section-wrap"><a className="back-link" href={`${localUrl('')}?lang=${lang}#works`}><ArrowLeft size={17}/>{t.back}</a><div className="detail-heading"><div><p className="eyebrow">TDDD / {t[work.category]}</p><h1>{title}</h1></div><span className="detail-count">{String(work.images.length).padStart(2,'0')}<small>{t.images}</small></span></div><div className="detail-description"><span>{t.overview}</span><p>{translated(work.description,lang)}</p></div></section>
 <Dialog open={selected!==null} onOpenChange={open=>{if(!open)setSelected(null)}}>
 <section className={`detail-gallery section-wrap category-${work.category}`} aria-label={t.gallery}>{work.images.map((img,i)=><figure key={`${img.src}-${i}`} className="detail-figure"><DialogTrigger render={<button className="gallery-image"/>} onClick={()=>setSelected(i)} aria-label={`${t.original} — ${img.alt||title}`}><img src={mediaUrl(img.src)} alt={img.alt||title} width={img.width} height={img.height} loading={i===0?'eager':'lazy'} decoding="async"/><span className="expand-image"><Maximize2 size={18}/>{t.original}</span></DialogTrigger><figcaption><span>{String(i+1).padStart(2,'0')} / {String(work.images.length).padStart(2,'0')}</span><span>{title}</span></figcaption></figure>)}</section>
 ...
 <div className="lightbox-top"><div><DialogTitle>{title}</DialogTitle><DialogDescription className="sr-only">{t.original}</DialogDescription></div><DialogClose render={<Button variant="ghost" className="lightbox-close" aria-label={t.close}/>}><X size={23}/></DialogClose></div>
 ...
 {next&&<a className="next-project section-wrap" href={`${localUrl(`work/${next.slug}/`)}?lang=${lang}`}><div><p className="eyebrow">{t.nextWork}</p><h2>{translated(next.title,lang)}</h2></div><ArrowUpRight size={54}/></a>}
