...
export default function WorkDetail({work,next}:{work:Work;next:Work|null}) {
 ...
 <main id="main"><section className={`detail-intro section-wrap ${showText?'':'image-only-intro'}`}><a className="back-link" href={`${localUrl('')}?lang=${lang}#works`}><ArrowLeft size={17}/>{t.back}</a>{showText?<><div className="detail-heading"><div><h1>{title}</h1></div><span className="detail-count">{String(work.images.length).padStart(2,'0')}<small>{t.images}</small></span></div><div className="detail-description"><span>{t.overview}</span><p>{translated(work.description,lang)}</p></div></>:<h1 className="sr-only">{title}</h1>}</section>
